...
uint8_t ecdsaSign(BigNum256 r, BigNum256 s, const BigNum256 hash, const BigNum256 private_key)
{
	PointAffine big_r;
	uint8_t k[32];
   ...
   //swapEndian256(&(seed_material[32])); // little-endian -> big-endian
	...
	while (true)
	{
        ...
        //printf("big_r.y1 : "); // decide v at this line is precise
        //printUint(big_r.y, 32);
        
        v = (big_r.y[0] % 0x02); // this should be fixed
        uint8_t v2 = bigCompare(big_r.x, r) ? 0 : 2;
        v = v | v2;
        ...
        //uint8_t chainID = 76;
        //uint8_t eip155v = chainID * 2 + 35 + v;
        //ecdsaSerialise(serialisedSig, r, true);
        return v;
		break;
	}
}

PointAffine privateKeyToPublicKey(BigNum256 private_key) {
    PointAffine big_r;
